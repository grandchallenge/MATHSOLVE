...
def fail(msg):
    raise SystemExit('GCL BSD-R5-RES preflight: FAIL: ' + msg)
...
def main():
    if len(sys.argv) != 3 or sys.argv[1:] != ['BSD-001', 'BSD-R5-RES']:
        fail('expected BSD-001 BSD-R5-RES')
    state = load('.gcl/campaigns/BSD-001/CAMPAIGN_STATE.json')
    op = load('.gcl/operations/BSD-R5-RES/OPERATION.json')
    fr = load('.gcl/admission/BSD-R5-RES/FREEZE.json')
    if state['current_operation'] != 'BSD-R5-RES' or op['operation'] != 'BSD-R5-RES':
    ...
    frozen = fr['artifacts']
    governed = op['governed_artifacts']
    if set(frozen) != set(governed):
        fail('freeze set mismatch')
    for rel in governed:
        if not (ROOT / rel).exists():
            fail('missing ' + rel)
        if blob(rel) != frozen[rel]:
