...
def main():
    ...
    if state['current_operation']!='BSD-R5-PRIM' or op['operation']!='BSD-R5-PRIM': fail('operation mismatch')
    if state['current_frontier']['id']!=op['objective']['frontier']: fail('frontier mismatch')
    if state['candidate_disposition']['type']!=op['candidate_disposition']: fail('disposition mismatch')
    if state['candidate_disposition']['blocked_on']!=op['blocked_on']: fail('blocked frontier mismatch')
    ...
    for k,v in op['claim_firewall'].items():
        ...
        if v is True and f'{k}: true' not in ledger: fail('protected truth '+k)
