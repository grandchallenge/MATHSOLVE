...
def fail(msg):
    raise SystemExit('GCL BSD-R5-LIFT-EQUIV preflight: FAIL: '+msg)
...
def main():
    if len(sys.argv)!=3 or sys.argv[1:]!=['BSD-001','BSD-R5-LIFT-EQUIV']:
        fail('expected BSD-001 BSD-R5-LIFT-EQUIV')
    state=load('.gcl/campaigns/BSD-001/CAMPAIGN_STATE.json')
    op=load('.gcl/operations/BSD-R5-LIFT-EQUIV/OPERATION.json')
    fr=load('.gcl/admission/BSD-R5-LIFT-EQUIV/FREEZE.json')
    if state['current_operation']!='BSD-R5-LIFT-EQUIV' or op['operation']!='BSD-R5-LIFT-EQUIV': fail('operation mismatch')
    if state['current_frontier']['id']!=op['objective']['retire_frontier']: fail('frontier mismatch')
    if state['current_frontier']['parent']!=op['objective']['parent_frontier']: fail('parent frontier mismatch')
    ...
    print(json.dumps({
        ...
        'candidate_disposition':op['candidate_disposition'],
        'frozen_artifacts':len(governed),
        'r5_lift_candidate':True,
        'r5_prim':False,
        'bsd_r2_a1':False,
