...
def fail(msg): raise SystemExit('GCL BSD-R5-RECIP preflight: FAIL: '+msg)
def load(rel):
    p=ROOT/rel
...
def main():
    if len(sys.argv)!=3 or sys.argv[1:]!=['BSD-001','BSD-R5-RECIP']:
        fail('expected BSD-001 BSD-R5-RECIP')
    state=load('.gcl/campaigns/BSD-001/CAMPAIGN_STATE.json')
    op=load('.gcl/operations/BSD-R5-RECIP/OPERATION.json')
    fr=load('.gcl/admission/BSD-R5-RECIP/FREEZE.json')
    if state['current_operation']!='BSD-R5-RECIP' or op['operation']!='BSD-R5-RECIP': fail('operation mismatch')
    if state['current_frontier']['id']!=op['objective']['frontier']: fail('frontier mismatch')
    if state['candidate_disposition']['type']!=op['candidate_disposition'] or op['candidate_disposition']!='CLOSED': fail('disposition mismatch')
    if state['candidate_disposition']['next_frontier_if_protected']!=op['successor_policy']['next_frontier_if_closed']: fail('successor mismatch')
    ...
    if fr['provider_anchor']!=op['provider_anchor']: fail('freeze provider mismatch')
    ...
    for key,value in op['claim_firewall'].items():
        if value is False and f'{key}: false' not in ledger: fail('firewall '+key)
        if value is True and f'{key}: true' not in ledger: fail('protected truth '+key)
    ...
    print(json.dumps({'status':'PASS','campaign':'BSD-001','operation':'BSD-R5-RECIP','candidate_disposition':'CLOSED','next_frontier':op['successor_policy']['next_frontier_if_closed'],'frozen_artifacts':len(op['governed_artifacts']),'r5_lift':True,'r5_prim':False,'r5_res':False,'bsd_r2_a1':False,'mathcert_certified':False},sort_keys=True))
