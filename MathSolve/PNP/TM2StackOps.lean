...
theorem embedTM2TokenStack_push {tm : Turing.FinTM2}
  ...
  · simp only [hpos, if_false]
    ...
    · have hnotLeft : ¬ head ≤ position := by omega
      have hnotRight : ¬ head ≤ position + 1 := by omega
      simp [embedTM2TokenStack, hnotLeft, hnotRight]
    · have hle : head ≤ position := by omega
      have hleSucc : head ≤ position + 1 := by omega
      have hoff :
          (position - (head - 1)).toNat = (position - head).toNat + 1 := by
        omega
      simp [embedTM2TokenStack, hle, hleSucc, hoff,
...
theorem embedTM2TokenStack_pop {tm : Turing.FinTM2}
  ...
  · simp only [hpos, if_false]
    by_cases hlt : position < head
    · have hnotLeft : ¬ head ≤ position := by omega
      have hnotRight : ¬ head < position := by omega
      simp [embedTM2TokenStack, hnotLeft, hnotRight]
    · have hstrict : head < position := by omega
      have hle : head ≤ position := by omega
      have hoff :
          (position - head).toNat = (position - (head + 1)).toNat + 1 := by
        omega
      simp [embedTM2TokenStack, hle, hstrict, hoff,
        List.getElem?_cons_succ]
