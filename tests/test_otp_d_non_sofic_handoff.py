...
SPEC = importlib.util.spec_from_file_location("validate_otp_d_non_sofic_handoff", ROOT / "ci/validate_otp_d_non_sofic_handoff.py")
mod = importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(mod)


class DNonSoficHandoffTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_reject_source_verbatim_inflation(self):
        self.reject(lambda r: r["target_scope"]["classifications"].__setitem__(0, "source_faithful_exact_projection"))

    def test_reject_semantic_blob_substitution(self):
        self.reject(lambda r: r["authority"]["semantic_record"].__setitem__("digest", "0" * 40))

    def test_reject_mathlib_identity_drift(self):
        self.reject(lambda r: r["authority"]["official_subject"].__setitem__("mathlib_finite_presentation_blob", "0" * 40))
