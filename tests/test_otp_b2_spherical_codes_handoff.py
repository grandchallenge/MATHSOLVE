...
SPEC=importlib.util.spec_from_file_location("validate_otp_b2_spherical_codes_handoff",ROOT/"ci/validate_otp_b2_spherical_codes_handoff.py")
mod=importlib.util.module_from_spec(SPEC)
assert SPEC and SPEC.loader
SPEC.loader.exec_module(mod)

class B2SphericalCodesHandoffTests(unittest.TestCase):
    ...
    def test_predecessor_authority(self): self.reject(lambda r:r["target_scope"].__setitem__("predecessor_seven_target_surface_authorized",True))
    def test_numerical_classification_inflation(self): self.reject(lambda r:r["target_scope"]["classifications"].__setitem__(3,"source_faithful_exact_projection"))
    ...
    def test_exact_039661_source_inflation(self): self.reject(lambda r:r["route_controls"].__setitem__("exact_039661_source_verbatim",True))
