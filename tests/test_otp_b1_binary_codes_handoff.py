...
SPEC = importlib.util.spec_from_file_location(
    "validate_otp_b1_binary_codes_handoff",
    ROOT / "ci/validate_otp_b1_binary_codes_handoff.py",
)
...
class B1BinaryCodesHandoffTests(unittest.TestCase):
    def setUp(self):
    ...
    def test_reject_target_substitution(self):
        self.reject(lambda r: r["target_scope"]["lean_theorems"].__setitem__(0, "Other.Target"))

    def test_reject_target_reordering(self):
        self.reject(lambda r: r["target_scope"]["lean_theorems"].reverse())

    def test_reject_classification_inflation(self):
        self.reject(lambda r: r["target_scope"]["classifications"].__setitem__(1, "source_faithful_exact_projection"))

    def test_reject_semantic_blob_substitution(self):
        self.reject(lambda r: r["authority"]["semantic_record"].__setitem__("digest", "0" * 40))

    def test_reject_formal_root_substitution(self):
        self.reject(lambda r: r["authority"]["official_subject"].__setitem__("commit", "0" * 40))
