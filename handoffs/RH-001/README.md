...
10. The full localized Weil operator is now controlled analytically near \(\lambda=1\). RH-R040 gives an explicit simple-even interval \(0<a=\log\lambda\le1/16\) and strict positivity through \(a\le1/50\). RH-R045 sharpens the structural interval to \(0<a\le2/15\). RH-R046 strengthens the limiting odd-sector coercivity and extends the structural theorem to \(a\le1/7\). RH-R047 then refines the integrated smooth-remainder estimate and extends the full simple-even theorem to
\[
0<a\le\frac3{20}.
\]
Uniformly on that interval,
\[
\epsilon_-(e^a)-\epsilon_+(e^a)>\frac{641}{11600},
\]
and
\[
\epsilon_{+,2}(e^a)-\epsilon_{+,1}(e^a)>\frac1{1624}.
\]
Thus the first CCM simple-even obstruction is fully discharged through \(a=3/20\). The current explicit bottleneck is the even-sector internal-gap certificate, not the odd parity bound.
...
6. RH-R047 uses an integrated remainder envelope to preserve \(0\le q'(t)\le1/5\) through \(t=3/10\) and reaches \(a=3/20\).

At the new endpoint the certified parity gap still has material slack, while the even internal-gap lower bound is only \(1/1624\). The smallest next theorem should therefore improve the even-sector certificate. Preferred routes are:

- sharpen the lower bound for the second even limiting eigenvalue beyond \(3/4\);
- enlarge the positive even trial space to lower the ground-state upper bound;
- sharpen the even residual estimate below \(2a^2/5\);
- or use the protected \(d/\Delta_H\) continuation interfaces if they yield a shorter quantitative bridge.
